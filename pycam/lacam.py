...
class LaCAM:
    ...
    def solve(
        ...
        seed: int = 0,
        verbose: int = 1,
        restrict_directions: bool = False,
    ) -> Configs:
        # set problem
        ...
        if restrict_directions:
            print("Running with restricted directions")
            self.restricted_directions = preprocess_map(grid)
            for g in self.goals:
                for n in get_neighbors(self.grid, g, ["left", "right", "up", "down"]):
                    self.restricted_directions[n] = ["left", "right", "up", "down"]
        else:
            print("Running without restricted directions")
    ...
    def _solve(self) -> Configs:
        ...
        # set distance tables
        self.dist_tables = [DistTable(self.grid, g, self.restricted_directions) for g in self.goals]

        # set PIBT
        self.pibt = PIBT(self.dist_tables, self.restricted_directions)
        ...
        # set initial node
        Q_init = self.starts
        N_init = HighLevelNode(Q=Q_init, order=self.get_order(Q_init), h=self.get_h_value(Q_init))
        ...
        while len(OPEN) > 0 and not self.deadline.is_expired:
            ...
            if C.depth < self.num_agents:
                i = N.order[C.depth]
                v = N.Q[i]
                directions = self.restricted_directions.get(v, ["left", "right", "up", "down"])
                if len(directions) == 0:
                    directions = ["left", "right", "up", "down"]
                cands = [v] + get_neighbors(self.grid, v, directions)
    ...
    def configuration_generator(self, N: HighLevelNode, C: LowLevelNode) -> Config | None:
