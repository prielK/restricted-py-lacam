...
class PIBT:
    def __init__(
        self, dist_tables: list[DistTable], restricted_directions: dict, seed: int = 0
    ) -> None:
    ...
    def funcPIBT(self, Q_from: Config, Q_to: Config, i: int) -> bool:
        ...
        directions = self.restricted_directions.get(Q_from[i], ["left", "right", "up", "down"])
        if len(directions) == 0:
            directions = ["left", "right", "up", "down"]
        # get candidate next vertices
        C = [Q_from[i]] + get_neighbors(self.grid, Q_from[i], directions)
