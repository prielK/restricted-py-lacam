...
def get_neighbors(grid: Grid, coord: Coord, directions: dict) -> list[Coord]:
    # coord: y, x
    neigh: list[Coord] = []
    ...
    if x > 0 and grid[y, x - 1] and "left" in directions:
        neigh.append((y, x - 1))

    if x < grid.shape[1] - 1 and grid[y, x + 1] and "right" in directions:
        neigh.append((y, x + 1))

    if y > 0 and grid[y - 1, x] and "up" in directions:
        neigh.append((y - 1, x))

    if y < grid.shape[0] - 1 and grid[y + 1, x] and "down" in directions:
        neigh.append((y + 1, x))
...
def validate_mapf_solution(
    ...
    for t in range(T):
        for i in range(N):
            ...
            directions = restricted_directions.get(v_i_pre, ["left", "right", "up", "down"])
            if len(directions) == 0:
                directions = ["left", "right", "up", "down"]
            # check continuity
            assert v_i_now in [v_i_pre] + get_neighbors(
                grid, v_i_pre, directions
